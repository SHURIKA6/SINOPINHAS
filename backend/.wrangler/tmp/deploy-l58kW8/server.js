...
app.post("/api/admin/login", async (c) => {
  ...
  app.get("/api/admin/users", async (c2) => {
    const adminPasswordFromQuery = c2.req.query("admin_password");
    const env2 = c2.env;
    if (adminPasswordFromQuery !== env2.ADMIN_PASSWORD) {
      return c2.json({ error: "Senha de admin incorreta" }, 403);
    }
    try {
      const { rows } = await queryDB("SELECT id, username, bio, created_at, avatar FROM users ORDER BY id DESC LIMIT 50", [], env2);
      return c2.json(rows);
    } catch (err) {
      return c2.json({ error: "Erro ao listar usu\xE1rios: " + err.message }, 500);
    }
  });
  app.get("/api/admin/logs", async (c2) => {
    const adminPasswordFromQuery = c2.req.query("admin_password");
    const env2 = c2.env;
    if (adminPasswordFromQuery !== env2.ADMIN_PASSWORD) {
      return c2.json({ error: "Acesso Negado: Credenciais Inv\xE1lidas" }, 403);
    }
    try {
      ...
      return c2.json({ error: "Erro ao buscar logs: " + err.message }, 500);
    }
  });
});
