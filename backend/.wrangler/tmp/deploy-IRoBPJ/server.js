...
app.post("/api/upload", async (c) => {
  ...
  try {
    ...
    console.log(`\u{1F511} API Key (8 primeiros): ${env2.BUNNY_API_KEY?.substring(0, 8)}`);
    ...
    const createVideoRes = await fetch(
      `https://video.bunnycdn.com/library/${env2.BUNNY_LIBRARY_ID}/videos`,
      {
        ...
        headers: {
          ...
          // ← Formato 1
          "Authorization": `Bearer ${env2.BUNNY_API_KEY}`,
          // ← Formato 2 (backup)
    ...
    console.log(`\u{1F4E1} Resposta Bunny (${createVideoRes.status}):`, responseText.substring(0, 200));
    ...
    console.log(`\u2705 V\xEDdeo criado: ${videoGuid}`);
    ...
    console.log(`\u{1F4E1} Upload status: ${uploadRes.status}`);
    if (!uploadRes.ok) {
