...
var fetch, Headers2, Request2, Response2, AbortController, FetchError, AbortError, redirectStatus, isRedirect, node_fetch_default;
var init_node_fetch = __esm({
  "node_modules/unenv/dist/runtime/npm/node-fetch.mjs"() {
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    fetch = /* @__PURE__ */ __name((...args) => globalThis.fetch(...args), "fetch");
...
async function logAudit(user_id, action, meta = {}, c) {
  try {
    ...
    const platform2 = meta?.platform || null;
    let isp = null;
    if (cfASN) {
      isp = `ASN ${cfASN}`;
    }
...
app.post("/api/upload", async (c) => {
  ...
  try {
    ...
    console.log(`\u{1F4E4} Upload iniciado: "${title2}" por User ID ${userId}`);
    const { default: fetch2 } = await Promise.resolve().then(() => (init_node_fetch(), node_fetch_exports));
    const FormData = (await Promise.resolve().then(() => __toESM(require_browser(), 1))).default;
    const form = new FormData();
    const buffer = await file.arrayBuffer();
    form.append("file", Buffer.from(buffer), file.name);
    const uploadRes = await fetch2(
    ...
    console.log(`\u2705 V\xEDdeo enviado ao Google Drive: ${fileId}`);
    let thumbnailUrl = null;
    if (thumbnail) {
      const thumbBuffer = await thumbnail.arrayBuffer();
      const thumbForm = new FormData();
      thumbForm.append("file", Buffer.from(thumbBuffer), thumbnail.name);
      const thumbRes = await fetch2(
        `https://sinopinhas-gdrive-upload.fernandoriga.workers.dev/upload`,
    ...
    await queryDB(
      ...
      [title2, fileId, userId, isRestricted, thumbnailUrl],
      env2
    );
