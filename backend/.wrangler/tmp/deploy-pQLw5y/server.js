...
app.delete("/api/videos/:id", async (c) => {
  ...
  try {
    ...
    if (adminPassword === env2.ADMIN_PASSWORD) {
      result = await queryDB("DELETE FROM videos WHERE id = $1 RETURNING id", [videoId], env2);
    } else {
      result = await queryDB("DELETE FROM videos WHERE id = $1 AND user_id = $2 RETURNING id", [videoId, parseInt(userId)], env2);
    }
    if (result.rowCount === 0) {
