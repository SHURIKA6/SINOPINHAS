...
async function compare(password, hashedPassword) {
  const computedHash = await hash2(password);
  return computedHash === hashedPassword;
}
__name(compare, "compare");
async function queryDB(sql, params = [], env2) {
  try {
    const { Pool: Pool2 } = await Promise.resolve().then(() => (init_esm(), esm_exports));
    const pool = new Pool2({ connectionString: env2.DATABASE_URL });
...
async function logAudit(user_id, action, meta = {}, c) {
  try {
    ...
    if (userAgent.match(/iPhone/i))
      ...
      deviceType = "iPad";
    else if (userAgent.match(/Android.*Mobile/i))
      deviceType = "Android Mobile";
    else if (userAgent.match(/Android/i))
      deviceType = "Android Tablet";
    else if (userAgent.match(/Mobile|Tablet/i))
      deviceType = "Mobile";
    ...
    try {
      ...
      try {
        ...
        console.log(`\u26A0\uFE0F LOG B\xC1SICO salvo: ${action}`);
      } catch (fallbackError) {
        console.error("\u274C Falha total ao salvar log:", fallbackError.message);
      }
    }
  } catch (err) {
    console.error("\u26A0\uFE0F Falha ao gravar log (n\xE3o cr\xEDtico):", err.message);
  }
}
...
app.post("/api/register", async (c) => {
  ...
  try {
    const body = await c.req.json();
    console.log("\u{1F4E6} Body recebido no registro:", JSON.stringify(body, null, 2));
    ...
    console.log(`\u{1F50D} Verificando se "${username}" existe...`);
    ...
    if (existing.length > 0) {
      console.log(`\u274C Usu\xE1rio "${username}" j\xE1 existe`);
      await logAudit(null, "REGISTER_FAILED_USERNAME_EXISTS", { username, ...body }, c);
      return c.json({ error: "Usu\xE1rio j\xE1 existe" }, 400);
    }
    console.log("\u{1F510} Gerando hash da senha...");
    const hashedPassword = await hash2(password);
    console.log("\u{1F4BE} Inserindo usu\xE1rio no banco...");
    ...
    const user = rows[0];
    console.log(`\u2705 Usu\xE1rio criado com sucesso: ${username} (ID: ${user.id})`);
    try {
      await logAudit(user.id, "USER_REGISTERED", body, c);
    } catch (logErr) {
      console.error("\u26A0\uFE0F Erro ao salvar log (n\xE3o cr\xEDtico):", logErr.message);
    }
    return c.json({ user });
  } catch (err) {
    console.error("\u274C ERRO CR\xCDTICO AO REGISTRAR:", err);
...
app.post("/api/login", async (c) => {
  ...
  try {
    const body = await c.req.json();
    console.log("\u{1F4E6} Body recebido no login:", JSON.stringify(body, null, 2));
    const username = body.username;
    const password = body.password;
    if (!username || !password) {
    ...
    console.log(`\u{1F50D} Buscando usu\xE1rio: "${username}"`);
    const { rows } = await queryDB(
      "SELECT * FROM users WHERE username = $1",
      [username],
      env2
    ...
    if (rows.length === 0) {
      console.log(`\u274C Usu\xE1rio "${username}" n\xE3o encontrado`);
      await logAudit(null, "LOGIN_FAILED_USER_NOT_FOUND", { username, ...body }, c);
      return c.json({ error: "Usu\xE1rio ou senha incorretos" }, 401);
    }
    const user = rows[0];
    console.log(`\u{1F510} Verificando senha para usu\xE1rio ID: ${user.id}`);
    const validPassword = await compare(password, user.password);
    if (!validPassword) {
      console.log(`\u274C Senha incorreta para usu\xE1rio: ${username}`);
    ...
    console.error("\u274C ERRO CR\xCDTICO AO FAZER LOGIN:", err);
    console.error("Stack trace:", err.stack);
    return c.json({ error: "Erro no servidor: " + err.message }, 500);
  }
});
...
app.post("/api/upload", async (c) => {
  const env2 = c.env;
  try {
    console.log("\u{1F4E4} Iniciando upload para Bunny CDN...");
    ...
    if (!file || !title2 || !userId) {
      console.log("\u274C Faltam dados obrigat\xF3rios");
      return c.json({ error: "Faltam dados obrigat\xF3rios" }, 400);
    }
    console.log(`\u{1F4E4} Upload: "${title2}" (${file.size} bytes)`);
    console.log(`\u{1F511} BUNNY_API_KEY existe: ${!!env2.BUNNY_API_KEY}`);
    console.log(`\u{1F4DA} BUNNY_LIBRARY_ID: ${env2.BUNNY_LIBRARY_ID}`);
    console.log(`\u{1F511} Primeiros 8 caracteres da key: ${env2.BUNNY_API_KEY ? env2.BUNNY_API_KEY.substring(0, 8) : "UNDEFINED"}`);
    const createVideoRes = await fetch(
      `https://video.bunnycdn.com/library/${env2.BUNNY_LIBRARY_ID}/videos`,
      {
        method: "POST",
        headers: {
          "AccessKey": env2.BUNNY_API_KEY,
    ...
    if (!createVideoRes.ok) {
      ...
      console.error("\u274C Erro ao criar v\xEDdeo no Bunny:", errorText);
      throw new Error("Falha ao criar v\xEDdeo no Bunny: " + errorText);
    }
    const videoData = await createVideoRes.json();
    const videoGuid = videoData.guid;
    console.log(`\u2705 V\xEDdeo criado no Bunny: ${videoGuid}`);
    const buffer = await file.arrayBuffer();
    const uploadRes = await fetch(
      `https://video.bunnycdn.com/library/${env2.BUNNY_LIBRARY_ID}/videos/${videoGuid}`,
      {
        method: "PUT",
        headers: {
          "AccessKey": env2.BUNNY_API_KEY,
    ...
    if (!uploadRes.ok) {
      ...
      console.error("\u274C Erro ao fazer upload do v\xEDdeo:", errorText);
      throw new Error("Falha no upload: " + errorText);
    }
    console.log(`\u2705 Upload conclu\xEDdo para o Bunny`);
    await queryDB(
      "INSERT INTO videos (title, bunny_id, user_id, is_restricted) VALUES ($1, $2, $3, $4)",
      [title2, videoGuid, userId, isRestricted],
      env2
    );
    await logAudit(userId, "VIDEO_UPLOADED", { title: title2, is_restricted: isRestricted }, c);
    console.log(`\u2705 V\xEDdeo salvo no banco: "${title2}"`);
...
app.get("/api/videos", async (c) => {
  ...
  try {
    ...
    console.log(`\u2705 Listados ${rows.length} v\xEDdeos p\xFAblicos`);
    return c.json(rows);
  } catch (err) {
    console.error("\u274C Erro ao buscar v\xEDdeos:", err);
    return c.json({ error: "Erro ao buscar v\xEDdeos" }, 500);
  }
});
app.get("/api/secret-videos", async (c) => {
  ...
  try {
    ...
    console.log(`\u2705 Listados ${rows.length} v\xEDdeos restritos`);
    return c.json(rows);
  } catch (err) {
    console.error("\u274C Erro ao buscar v\xEDdeos restritos:", err);
    return c.json({ error: "Erro ao buscar v\xEDdeos" }, 500);
  }
});
app.delete("/api/videos/:id", async (c) => {
  ...
  try {
    ...
    const isAdmin = adminPassword === env2.ADMIN_PASSWORD;
    if (!isAdmin && !userId) {
      return c.json({ error: "N\xE3o autorizado" }, 403);
    }
    if (!isAdmin) {
      const { rows } = await queryDB(
      ...
      if (rows.length === 0) {
        return c.json({ error: "V\xEDdeo n\xE3o encontrado" }, 404);
      }
      if (rows[0].user_id.toString() !== userId.toString()) {
        return c.json({ error: "N\xE3o autorizado" }, 403);
      }
    }
    await queryDB("DELETE FROM videos WHERE id = $1", [videoId], env2);
    await logAudit(userId || null, "VIDEO_DELETED", { video_id: videoId, is_admin: isAdmin }, c);
    console.log(`\u2705 V\xEDdeo deletado: ID ${videoId}`);
    return c.json({ success: true });
  } catch (err) {
    console.error("\u274C Erro ao deletar v\xEDdeo:", err);
    return c.json({ error: "Erro ao deletar v\xEDdeo" }, 500);
  }
});
app.post("/api/videos/:id/like", async (c) => {
  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      console.log(`\u{1F494} Like removido: V\xEDdeo ${videoId} por User ${user_id}`);
    } else {
      await queryDB(
      ...
      console.log(`\u2764\uFE0F Like adicionado: V\xEDdeo ${videoId} por User ${user_id}`);
    }
    return c.json({ success: true });
  } catch (err) {
    console.error("\u274C Erro ao curtir v\xEDdeo:", err);
    return c.json({ error: "Erro ao curtir v\xEDdeo" }, 500);
  }
});
app.post("/api/videos/:id/view", async (c) => {
  ...
  try {
    ...
    console.log(`\u{1F441}\uFE0F View registrada: V\xEDdeo ${videoId} por User ${user_id}`);
    return c.json({ success: true });
  } catch (err) {
...
app.post("/api/comment", async (c) => {
  ...
  try {
    ...
    if (!comment || !comment.trim()) {
      return c.json({ error: "Coment\xE1rio vazio" }, 400);
    }
    await queryDB(
    ...
    if (video.length > 0 && video[0].user_id !== user_id) {
      await queryDB(
        ...
        [video[0].user_id, "comment", video_id, "Novo coment\xE1rio no seu v\xEDdeo"],
        env2
      );
    }
    console.log(`\u{1F4AC} Coment\xE1rio adicionado: V\xEDdeo ${video_id} por User ${user_id}`);
    return c.json({ success: true });
  } catch (err) {
    console.error("\u274C Erro ao adicionar coment\xE1rio:", err);
    return c.json({ error: "Erro ao adicionar coment\xE1rio" }, 500);
  }
});
app.get("/api/comments/:videoId", async (c) => {
  ...
  try {
    ...
    console.log(`\u2705 Listados ${rows.length} coment\xE1rios do v\xEDdeo ${videoId}`);
    return c.json(rows);
  } catch (err) {
    console.error("\u274C Erro ao buscar coment\xE1rios:", err);
    return c.json({ error: "Erro ao buscar coment\xE1rios" }, 500);
  }
});
app.delete("/api/comments/:id", async (c) => {
  ...
  try {
    ...
    const isAdmin = admin_password === env2.ADMIN_PASSWORD;
    if (!isAdmin) {
      const { rows } = await queryDB(
    ...
    await queryDB("DELETE FROM comments WHERE id = $1", [commentId], env2);
    console.log(`\u2705 Coment\xE1rio deletado: ID ${commentId}`);
    return c.json({ success: true });
  } catch (err) {
    console.error("\u274C Erro ao deletar coment\xE1rio:", err);
    return c.json({ error: "Erro ao deletar coment\xE1rio" }, 500);
  }
});
app.get("/api/notifications/:userId", async (c) => {
  ...
  try {
    ...
    console.log(`\u2705 Listadas ${rows.length} notifica\xE7\xF5es do User ${userId}`);
    return c.json(rows);
  } catch (err) {
...
app.get("/api/users/all", async (c) => {
  ...
  try {
    ...
    console.log(`\u2705 Listados ${rows.length} usu\xE1rios`);
    return c.json(rows);
  } catch (err) {
    console.error("\u274C Erro ao listar usu\xE1rios:", err);
    return c.json({ error: "Erro ao listar usu\xE1rios" }, 500);
  }
});
app.post("/api/send-message", async (c) => {
  ...
  try {
    ...
    console.log(`\u{1F4E8} Mensagem enviada: De User ${from_id} para User ${to_id}`);
    return c.json({ success: true });
  } catch (err) {
...
app.get("/api/admin/users", async (c) => {
  ...
  if (adminPassword !== env2.ADMIN_PASSWORD) {
    console.log("\u274C Tentativa de acesso admin n\xE3o autorizado");
    return c.json({ error: "N\xE3o autorizado" }, 403);
  }
  try {
    const { rows } = await queryDB(
    ...
    console.log(`\u2705 Admin listou ${rows.length} usu\xE1rios`);
    return c.json(rows);
  } catch (err) {
    console.error("\u274C Erro ao listar usu\xE1rios:", err);
    return c.json({ error: "Erro ao listar usu\xE1rios" }, 500);
...
app.post("/api/admin/reset-password", async (c) => {
  ...
  try {
    const { user_id, admin_password } = await c.req.json();
    if (admin_password !== env2.ADMIN_PASSWORD) {
      return c.json({ error: "N\xE3o autorizado" }, 403);
    }
    const hashedPassword = await hash2("123456");
    await queryDB(
...
app.delete("/api/admin/users/:userId", async (c) => {
  ...
  try {
    ...
    await queryDB("DELETE FROM videos WHERE user_id = $1", [userId], env2);
    await queryDB("DELETE FROM comments WHERE user_id = $1", [userId], env2);
    await queryDB("DELETE FROM likes WHERE user_id = $1", [userId], env2);
    await queryDB("DELETE FROM messages WHERE from_id = $1 OR to_id = $1", [userId], env2);
    await queryDB("DELETE FROM users WHERE id = $1", [userId], env2);
    await logAudit(null, "ADMIN_USER_BANNED", { target_user_id: userId }, c);
    console.log(`\u2705 Admin baniu User ${userId}`);
    return c.json({ success: true });
  } catch (err) {
    console.error("\u274C Erro ao banir usu\xE1rio:", err);
    return c.json({ error: "Erro ao banir usu\xE1rio" }, 500);
  }
});
app.get("/api/admin/logs", async (c) => {
  ...
  const adminPassword = c.req.query("admin_password");
  if (adminPassword !== env2.ADMIN_PASSWORD) {
    console.log("\u274C Tentativa de acesso aos logs n\xE3o autorizada");
    return c.json({ error: "N\xE3o autorizado" }, 403);
  }
  try {
    const { rows } = await queryDB(
