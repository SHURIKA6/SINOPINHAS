...
function mergeConfig(config1, config2) {
  ...
  const mergeMap = {
    ...
    headers: /* @__PURE__ */ __name((a2, b, prop) => mergeDeepProperties(headersToObject(a2), headersToObject(b), prop, true), "headers")
  };
...
async function queryDB(sql, params, env2) {
  if (!env2.DATABASE_URL) throw new Error("DATABASE_URL n\xE3o configurada.");
  const pool = new Zs({
...
async function logAudit(user_id, action, meta, c) {
  try {
    ...
    const safeUserId = typeof user_id === "number" || typeof user_id === "string" && !isNaN(user_id) ? parseInt(user_id) : null;
    await queryDB(
...
app.post("/api/register", async (c) => {
  ...
  const env2 = c.env;
  if (!username || !password) return c.json({ error: "Usu\xE1rio e senha obrigat\xF3rios" }, 400);
  try {
    ...
    if (errorMsg.includes("duplicate key")) {
      return c.json({ error: "Username j\xE1 existe" }, 409);
    }
...
app.post("/api/login", async (c) => {
  ...
  const env2 = c.env;
  const { rows } = await queryDB("SELECT * FROM users WHERE username = $1", [username.toLowerCase()], env2);
  if (!rows.length) return c.json({ error: "Usu\xE1rio/senha inv\xE1lido" }, 401);
  const user = rows[0];
  const ok = await comparePassword(password, user.password);
  if (!ok) return c.json({ error: "Usu\xE1rio/senha inv\xE1lido" }, 401);
...
app.get("/api/admin/users", async (c) => {
  ...
  const env2 = c.env;
  if (adminPasswordFromQuery !== env2.ADMIN_PASSWORD) {
    return c.json({ error: "Senha de admin incorreta" }, 403);
  }
  try {
    const { rows } = await queryDB("SELECT id, username, bio, created_at, avatar FROM users ORDER BY id DESC LIMIT 50", [], env2);
    return c.json(rows);
  } catch (err) {
    console.error("Erro ao listar usu\xE1rios:", err);
    return c.json({ error: "Erro ao listar usu\xE1rios: Falha no DB" }, 500);
  }
});
app.get("/api/admin/logs", async (c) => {
  ...
  const env2 = c.env;
  if (adminPasswordFromQuery !== env2.ADMIN_PASSWORD) {
    return c.json({ error: "Acesso Negado: Credenciais Inv\xE1lidas" }, 403);
  }
  try {
...
app.delete("/api/admin/users/:id", async (c) => {
  ...
  const env2 = c.env;
  if (admin_password !== env2.ADMIN_PASSWORD) return c.json({ error: "Acesso Negado" }, 403);
  try {
    await queryDB("DELETE FROM comments WHERE user_id = $1", [id], env2);
    await queryDB("DELETE FROM video_reactions WHERE user_id = $1", [id], env2);
    await queryDB("DELETE FROM videos WHERE user_id = $1", [id], env2);
    await queryDB("DELETE FROM audit_logs WHERE user_id = $1", [id], env2);
    const result = await queryDB("DELETE FROM users WHERE id = $1", [id], env2);
    if (result.rowCount === 0) {
      return c.json({ error: "Usu\xE1rio n\xE3o encontrado." }, 404);
    }
    return c.json({ success: true });
  } catch (err) {
    console.error("Erro fatal ao banir usu\xE1rio (DB):", err);
    return c.json({ error: "Erro interno ao banir usu\xE1rio." }, 500);
  }
});
app.post("/api/admin/reset-password", async (c) => {
  ...
  try {
    ...
    const result = await queryDB("UPDATE users SET password = $1 WHERE id = $2 RETURNING id", [hash, user_id], env2);
    if (result.rowCount === 0) {
      return c.json({ error: "Usu\xE1rio n\xE3o encontrado." }, 404);
    }
    return c.json({ success: true });
...
app.post("/api/upload", async (c) => {
  ...
  if (!user_id || !file || typeof file === "string") return c.json({ error: "Arquivo obrigat\xF3rio" }, 400);
  ...
  if (userCheck.rows.length === 0) {
    ...
    return c.json({ error: "Acesso negado. Fa\xE7a login para continuar." }, 401);
  }
  try {
    const createRes = await axios_default.post(
    ...
    await axios_default.put(
      ...
      file,
      // Passa o objeto File/Blob diretamente (ou use await file.arrayBuffer())
    ...
    await queryDB(
      ...
      [title2, parseInt(user_id), videoGuid, is_restricted === "true" || false],
      c.env
    );
    await logAudit(user_id, "UPLOAD_VIDEO", { title: title2, service: "BunnyCDN", restricted: is_restricted === "true" }, c);
    return c.json({ success: true });
  } catch (error3) {
    console.error("Erro no Upload:", error3.response?.data || error3.message);
    return c.json({ error: "Falha ao enviar v\xEDdeo para o servidor de streaming" }, 500);
  }
});
app.get("/api/secret/videos", async (c) => {
  try {
    ...
    return c.json({ error: "Erro ao buscar conte\xFAdo restrito" }, 500);
  }
});
...
app.delete("/api/videos/:id", async (c) => {
  ...
  const env2 = c.env;
  const isAuthorized = adminPassword === env2.ADMIN_PASSWORD || userId && !isNaN(parseInt(userId));
  if (!isAuthorized) {
    return c.json({ error: "Autoriza\xE7\xE3o necess\xE1ria" }, 401);
  }
  try {
    await queryDB("DELETE FROM comments WHERE video_id = $1", [videoId], env2);
    await queryDB("DELETE FROM video_reactions WHERE video_id = $1", [videoId], env2);
    const videoResult = await queryDB("SELECT bunny_id FROM videos WHERE id = $1", [videoId], env2);
    const bunnyId = videoResult.rows[0]?.bunny_id;
    if (bunnyId && env2.BUNNY_API_KEY && env2.BUNNY_LIBRARY_ID) {
      await axios_default.delete(
        `https://video.bunnycdn.com/library/${env2.BUNNY_LIBRARY_ID}/videos/${bunnyId}`,
        { headers: { AccessKey: env2.BUNNY_API_KEY } }
      );
    }
    let deleteQuery;
    ...
    const result = await queryDB(deleteQuery, adminPassword === env2.ADMIN_PASSWORD ? [videoId] : [videoId, parseInt(userId)], env2);
    if (result.rowCount === 0) {
      return c.json({ error: "V\xEDdeo n\xE3o encontrado ou acesso negado." }, 404);
    }
    return c.json({ success: true });
  } catch (error3) {
    console.error("Erro no DELETE de V\xEDdeo (Final):", error3);
    return c.json({ error: "Erro interno ao deletar o v\xEDdeo." }, 500);
  }
});
app.post("/api/mural", async (c) => {
  ...
  const env2 = c.env;
  try {
    let mural = await env2.MURAL_STORE.get("mural_messages", { type: "json" }) || [];
    const userQuery = await queryDB("SELECT username FROM users WHERE id = $1", [user_id], env2);
    const username = userQuery.rows[0]?.username || "An\xF4nimo";
    const newPost = { user_id, username, msg, created_at: (/* @__PURE__ */ new Date()).toISOString() };
    mural.push(newPost);
    if (mural.length > 40) mural = mural.slice(mural.length - 40);
    await env2.MURAL_STORE.put("mural_messages", JSON.stringify(mural));
...
app.get("/api/comments/:video_id", async (c) => {
  ...
  if (!video_id || isNaN(parseInt(video_id))) {
    return c.json({ error: "ID de v\xEDdeo inv\xE1lido" }, 400);
  }
  try {
    const { rows } = await queryDB(
    ...
    return c.json({ error: "Erro interno ao buscar coment\xE1rios" }, 500);
  }
});
app.post("/api/comment", async (c) => {
  ...
  try {
    ...
    console.error("Erro ao inserir coment\xE1rio:", err);
    return c.json({ error: "Erro ao enviar coment\xE1rio" }, 500);
  }
});
