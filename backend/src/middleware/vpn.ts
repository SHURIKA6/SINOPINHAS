...
const VPN_PROVIDER_ASNS: string[] = [
...
const VPN_SPECIFIC_KEYWORDS: string[] = [
...
async function checkVPNShodan(ip: string): Promise<boolean> {
    try {
...
export async function isVPN(ip: string, c: AppContext): Promise<boolean> {
    try {
        ...
        console.error("⚠️ Erro ao verificar VPN:", (err as Error).message);
        // Falha aberta - permitir acesso se houver erro
        return false;
    }
}

export async function blockVPN(c: AppContext, next: Next): Promise<Response | void> {
